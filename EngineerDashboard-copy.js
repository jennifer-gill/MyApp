...
function DashboardTab({ user }) {
  ...
  const getLocation = async () => {
    ...
    if (address && address.length > 0) {
      const a = address[0];
      locationText = `${a.name || ""} ${a.street || ""}, ${a.city || ""}, ${
        a.region || ""
      }, ${a.country || ""}`;
    }
    ...
    <ScrollView style={{ padding: 16 }}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.row}>
            {["Ticket", "Project", "Documentation", "PM", "Presales"].map((t) => (
              ...
              >
                <Text style={{ color: serviceType === t ? "#fff" : "#ccc" }}>
                  {t}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.row}>
            {["Remote", "Onsite"].map((m) => (
              ...
              >
                <Text style={{ color: supportMode === m ? "#fff" : "#ccc" }}>
                  {m}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <TouchableOpacity
        ...
        style={[
          styles.button,
          {
            backgroundColor: checkInLoading || checkOutLoading
              ? "#bdc3c7"
              : checkedInClient
              ? "#e74c3c"
              : "#9b59b6",
